  ...
  def setup: IO[Unit] = {
    ...
    def migrate: IO[Unit] = IO.blocking {
      Flyway.configure
        ...
        .table(
          "flyway"
        ) // ⚠️ "flyway" as migration table history (in 'currentSchema' see above)
        .locations("nanako/config/Db/flyway") // ".sql" files migration resource path
...
object DbConfig { implicit val impl: DbConfig = DbConfig() }
